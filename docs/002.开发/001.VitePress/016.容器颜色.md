...
permalink: /vitepress/016                # 固定链接
titleTag:                          # 标题标签（推荐、精华等）
categories:                            # 分类
  - 笔记
tags:                                  # 标签
  - VitePress
description: 修改 tip、warning、danger 等容器的颜色主题，替换为美观配色。VitePress 内置的提示容器颜色可能不够美观，参考 VuePress/Hope 主题的配色方案，可以将提示框、警告框和危险框改为更加和谐的配色。 
coverImg:                 # 封面图片
author:
  name: 我就是双灵
