...
/**
 * 从 public/mcoo/ 扫描所有头像文件
 * 不依赖是否投过稿——只要有头像文件就在候选列表里
 * 格式: { gameId, avatar }
 * - gameId: 游戏 ID(文件名去掉 .png)
 * - avatar: 完整 avatar URL
 *
 * 用途:SubmissionForm 游戏 ID 输入框的实时补全(新成员第一次投稿也能搜到)
 */
function generateAvatars(docsRoot: string): { gameId: string; avatar: string }[] {
  const avatarsDir = join(docsRoot, 'public', 'mcoo')
...
//              docs/public/mcoo/ 下所有 .png 头像
// 重新生成方式:dev 修改任意 .md / 添加头像文件自动触发 / build 自动触发
`
  const content =
    banner +
...
export function mrdsDataLoader(): Plugin {
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      // 监听头像文件变化(新成员头像、新建后自动出现在候选列表)
      server.watcher.add(join(docsRoot, 'public', 'mcoo', '**/*.png'))
      const regenerate = () => {
        writeDataFile(docsRoot)
